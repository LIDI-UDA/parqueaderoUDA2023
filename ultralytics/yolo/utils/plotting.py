...
class Annotator:
    ...
    def box_label(self, box, label='', color=(128, 128, 128), txt_color=(255, 255, 255)):
        ...
        if self.pil or not is_ascii(label):
            ...
            # ### MODIFICACIONES
            # MODIFICACION, SE COMENTA LA LÍNEA QUE DIBUJA EL RECTÁNGULO
            # cv2.rectangle(self.im, p1, p2, color, thickness=self.lw, lineType=cv2.LINE_AA) lINEA ORIGINAL
            cv2.rectangle(self.im, p1, p2, (255, 255, 255), thickness=2, lineType=cv2.LINE_AA)

            # ### MODIFICACIONES
            # MODIFICACION PARA DIBUJAR LOS CENTROIDES DE LAS IMAGENES
            l_aux = [int(box[0]), int(box[1]), int(box[2]), int(box[3])]
            centroides = fn.calcular_centroides(l_aux)
            cv2.circle(self.im, (centroides[0], centroides[1]), 5, (0, 0, 255), 5, cv2.FILLED)
