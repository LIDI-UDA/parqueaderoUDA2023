...
def verificar_area(nuevos_pt, df_clusters):
    df_cluster_dict = df_clusters.to_dict('records')  # Se pasa a diccionario por ser la forma mas eficiente segun
    # medium
    # cont = []
    cont = None
    for fil in df_cluster_dict:  # Recorro todos los centroides y radios del dataframe
        distancia = distancia_entre_puntos([fil['x_cen_cluster'], fil['y_cen_cluster']], nuevos_pt[0], nuevos_pt[1])
        # print(f"cent: {nuevos_pt[0]} {nuevos_pt[1]} radio: {fil['radio_cluster']} dist: {distancia} "
        # f"x_cen_c: {fil['x_cen_cluster']} y_cen_c: {fil['y_cen_cluster']}")
        if distancia <= fil['radio_cluster']:
            # cont.append(fil['Cluster'])
            # print(f"Se está ocupando el espacio: {fil['Cluster']}")
            # cont += 1
            cont = fil['Cluster']

    # print(cont)
    return cont
    # print("No ocupa ningun parqueadero")
    # print(f"Número de espacios disponibles: {espacios_disponibles}")


# Esta función dibuja los vértices del rectángulo
def dibujar_puntos_rectángulo(imag, coord):
